...
int main(int argc, char **argv) {
  ...
  TString out_path;
  if (run == "182013") {
    tag_path = "/eliza18/atlas/dryu/tmp/182013/data11_7TeV.00182013.calibration_VdM.merge.TAG.x123_m841_m848/merge.root";
    trp_path = "TriggerRates_ATLAS_182013.root";
    ts_paths.push_back("/eliza18/atlas/dryu/tmp/182013/timestamps/all_timestamps.dat");
    out_path = "dt_182013.root";

  } else if (run == "188951") {
    tag_path = "/eliza18/atlas/dryu/tmp/188951/data11_7TeV.00188951.calibration_VdM.merge.TAG.x123_m849_m848/merge.root";
    trp_path = "TriggerRates_ATLAS_188951.root";
    ts_paths.push_back("/eliza18/atlas/dryu/tmp/188951/timestamps/timestamps3.dat");
    out_path = "dt_188951.root";
  } else if (run == "191373") {
...
#define REDOTRPTIMESTAMPS
  ...
  TString tsfilename = "PlbTrpTimestamps_";
  tsfilename += run;
  tsfilename += ".dat";
  ...
  TString trp_ts_path = "PlbTrpTimestamps_";
  trp_ts_path += run;
  trp_ts_path += ".dat";
...
#define REDOTRP
  ...
  t_trp->SetBranchStatus("*", 0);
  t_trp->SetBranchStatus("L1_MBTS_2_BGRP7_TAP", 1);
  t_trp->SetBranchStatus("L1_MBTS_2_BGRP7_TAV", 1);
  t_trp->SetBranchStatus("TimeStamp", 1);
  t_trp->SetBranchStatus("L1_MBTS_2_BGRP7_PS", 1);

  Double_t current_TimeStamp;
  Float_t current_L1_MBTS_2_BGRP7_TAP;
  Float_t current_L1_MBTS_2_BGRP7_TAV;
  Float_t current_L1_MBTS_2_BGRP7_PS;
  Int_t current_pLB;

  t_trp->SetBranchAddress("L1_MBTS_2_BGRP7_TAP", &current_L1_MBTS_2_BGRP7_TAP);
  t_trp->SetBranchAddress("L1_MBTS_2_BGRP7_TAV", &current_L1_MBTS_2_BGRP7_TAV);
  t_trp->SetBranchAddress("L1_MBTS_2_BGRP7_PS", &current_L1_MBTS_2_BGRP7_PS);
...
#ifdef DEBUG
      if (i%1000 == 0) {
        cout << "Timestamp not found: " << current_TimeStamp << endl;
      }
#endif
      continue;
    }

    h_ts_ntrig_ap->Fill(current_TimeStamp-0.5, current_L1_MBTS_2_BGRP7_TAP);
    h_ts_ntrig_av->Fill(current_TimeStamp-0.5, current_L1_MBTS_2_BGRP7_TAV);
    //h_pLB_ntrig_ap->Fill(current_pLB, current_L1_MBTS_2_BGRP7_TAP);
    //h_pLB_ntrig_av->Fill(current_pLB, current_L1_MBTS_2_BGRP7_TAV);

    if (current_pLB != last_pLB) {
      h_pLB_ps->SetBinContent(current_pLB, current_L1_MBTS_2_BGRP7_PS);
    }
  ...
  TH1F *h_ts_ntrig_ap = (TH1F*)f_trp_in->Get("TsTriggersAP");
  h_ts_ntrig_ap->SetDirectory(0);
  TH1F *h_ts_ntrig_av = (TH1F*)f_trp_in->Get("TsTriggersAV");
  h_ts_ntrig_av->SetDirectory(0);
  TH1F *h_pLB_ps = (TH1F*)f_trp_in->Get("PlbPrescale");
  h_pLB_ps->SetDirectory(0);
  f_trp_in->Close();
#endif
...
#define REDOTAG
  ...
  //Declare histogram
  TH1F *h_ts_event_rate = new TH1F("TsEvents", ";ts;Events", n_timestamps, timestamp_min, timestamp_max);
  h_ts_event_rate->SetDirectory(0);
  ...
  for (int i=0; i<n_entries; i++) {
    if (i%100000 == 0) {
      cout << "Event " << i << " / " << n_entries <<endl;
    }
    t_tag->GetEntry(i);

    if (!(current_L1PassedTrigMaskTAV3 >> (selectItem%32) & 0x1)) {
      continue;
    }
    ...
    current_pLB = GetPlbFromTimestampTrp(current_timestamp);
    cout.precision(10);
    if ((total_triggers % 100000) == 0) {
      cout << "On pLB " << current_pLB << " / ts = " << current_timestamp << endl;
    }
    if (current_pLB < 0) {
      continue;
    }
  ...
  TH1F *h_ts_event_rate = (TH1F*)f_read_event_rates->Get("TsEvents");
  h_ts_event_rate->SetDirectory(0);
  ...
  for (map<Int_t, std::pair<Double_t, Double_t> >::iterator it = pLB_timestamps_trp.begin(); it != pLB_timestamps_trp.end(); ++it) {
    ...
    for (Int_t current_ts = ts_start+1; current_ts < ts_end; current_ts++) {
      ...
      if (h_ts_ntrig_ap->GetBinContent(h_ts_ntrig_ap->FindBin(current_ts)) < 1./1000.) {
        continue;
      }
...
Int_t GetPlbFromTimestamp( Double_t timestamp ) {
  for ( vector<Int_t>::iterator pLB = pLB_list.begin(); pLB != pLB_list.end(); ++pLB ) {
    if ((timestamp >= pLB_timestamps[*pLB].first) && (timestamp <= pLB_timestamps[*pLB].second)) {
      return *pLB;
    }
...
Int_t GetPlbFromTimestampTrp( Double_t timestamp ) {
  for ( vector<Int_t>::iterator pLB = pLB_list_trp.begin(); pLB != pLB_list_trp.end(); ++pLB ) {
    if ((timestamp > pLB_timestamps_trp[*pLB].first + 1.e-9) && (timestamp <= pLB_timestamps_trp[*pLB].second + 1.e-9)) {
      return *pLB;
    }
