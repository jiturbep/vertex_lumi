...
cout << "[initializePuCorr] Input path = "  << input_path << endl;
  ...
  if (input_type == "data") {
    ...
    for (vector<Int_t>::iterator bcid = bcids.begin(); bcid != bcids.end(); ++bcid) {
      ...
      h_z[*bcid]->Sumw2();
      TCanvas *canvas_z = new TCanvas(name,name,800,600);
      h_z[*bcid]->Draw("hist");
      canvas_z->Print(name+".pdf");
    ...
    bool first = true;
    TCanvas *canvas_dz = new TCanvas("h_dz_" + tag ,"h_dz_" + tag ,800,600);
    for (vector<Int_t>::iterator nTrkCut = nTrkCuts.begin(); nTrkCut != nTrkCuts.end(); ++nTrkCut) {
      ...
      for (vector<Int_t>::iterator bcid = bcids.begin(); bcid != bcids.end(); ++bcid) {
        ...
        for (vector<std::pair<Int_t, Int_t> >::iterator pLB_interval = good_pLBs.begin(); pLB_interval != good_pLBs.end(); ++pLB_interval) {
          ...
          cout << "[initializePuCorr] bin1 = " << bin1 << endl;
          cout << "[initializePuCorr] bin2 = " << bin2 << endl;
          h_dz->Add(h2->ProjectionX("tmp2", bin1, bin2));
        }
        h_dz->Sumw2();
        h_dz->Draw("hist");
        canvas_dz->Print(TString("h_dz_") + tag + ".pdf");
        canvas_dz->Clear();
       // delete canvas_dz;
        ...
        //h_dz_copy->Rebin(20);
      	...
      	f_dz->Close();
        //h_dz_copy->Draw("hist");
        //canvas_dz->Print(TString("h_dz_copy_") + tag + ".pdf");
        //canvas_dz->Clear();
        //canvas_dz_cp->Delete();
    ...
    for (std::vector<Int_t>::iterator nTrkCut = nTrkCuts.begin(); nTrkCut != nTrkCuts.end(); ++nTrkCut) {
      ...
      if (mc_energy == "7") {
        ...
        mu = 5; // HACK: change this to 0.5 once you have enough statistics at low mu to deal with it!
  		...
  		TString path_deltaz = "deltaz_NTrkCut"; path_deltaz+=*nTrkCut; path_deltaz+=".root";
  		TFile *f_deltaz;
      ...
      for (Int_t current_ngenint = 2; current_ngenint < 80; current_ngenint++) {
        ...
        //cout << "current_ngenint, " << current_ngenint << ", bin = " << bin << endl;
      ...
      TString path_z = "z_distribution_NTrkCut"; path_z+=*nTrkCut; path_z+=".root";
      TFile *f_z;
