// $Id: ChainGroup.cxx 470619 2011-11-25 12:50:28Z krasznaa $
...
namespace D3PD {
   ...
   ChainGroup::GetPassedL1Triggers( TrigDefs::DecisionTypes type ) {
      ...
      for( size_t i = 0; i < m_existingTriggers.size(); ++i ) {
         ...
         case TrigDefs::L1_TAP:
            ...
            break;
         default:
            Error( "GetPassedL1Triggers", "Decision type not recognised for LVL1 items!" );
            return result;
            break;
         }
      }
   ...
   Bool_t ChainGroup::Update() {
      ...
      // Check if the trigger names need updating:
      const ::Bool_t needUpdate = ( m_smk == m_parent.GetSMK() ) ? kFALSE : kTRUE;

      // Load the configuration, and remember that the object is initialized with this
      // configuration:
      if( ! m_configSvc.Load( m_parent.GetSMK(), m_parent.GetL1PSK(),
                              m_parent.GetHLTPSK() ) ) {
         Error( "Update", "Couldn't load configuration with SMK: %i, "
                "L1PSK: %i, HLTPSK: %i", m_parent.GetSMK(), m_parent.GetL1PSK(),
                m_parent.GetHLTPSK() );
         return kFALSE;
      }
      m_smk = m_parent.GetSMK();

      // Only continue continue if an update to the ChainGroup is needed:
      if( ! needUpdate ) return kTRUE;
