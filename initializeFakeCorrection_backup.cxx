...
int main(int argc, char **argv) {
  ...
  for (vector<Int_t>::iterator nTrkCut = nTrkCuts.begin(); nTrkCut != nTrkCuts.end(); ++nTrkCut) {
    ...
    for (int bin = 1; bin <= h_NTrig_NGenInt[*nTrkCut]->GetNbinsX(); bin++) {
      if (h_NTrig_NGenInt[*nTrkCut]->GetBinContent(bin) < 100) {
        cout << "Ever here? " << h_NTrig_NGenInt[*nTrkCut]->GetBinCenter(bin) << endl;
        mu_limit = h_NTrig_NGenInt[*nTrkCut]->GetBinCenter(bin);
        break;
      } else {
        if (tag == "mc_8TeV_17.2_VtxLumi_2newsets") {mu_limit = 110;}
        else{ mu_limit = 22; }
      }
      mu_limit = 110;
    }
  ...
  for (vector<Int_t>::iterator nTrkCut = nTrkCuts.begin(); nTrkCut != nTrkCuts.end(); ++nTrkCut) {
    ...
    cout << "Setting marker color to " << nTrkColors[*nTrkCut] << endl;
    tg_fake_fraction_MuReconMC[*nTrkCut]->SetMarkerStyle(20);
    cout << "Setting marker style to 20 " << endl;
